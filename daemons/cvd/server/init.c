...
int cvd_initialize_server(struct gracht_server_configuration* config, gracht_server_t** serverOut)
{
...
#if defined(__linux__)
    status = platform_mkdir("/run/chef/cvd");
    if (status) {
        fprintf(stderr, "cvd_initialize_server: failed to create /run/chef/cvd\n");
        return status;
    }
    status = platform_mkdir("/run/chef/cvd/clients");
    if (status) {
        fprintf(stderr, "cvd_initialize_server: failed to create /run/chef/cvd/clients\n");
        return status;
    }
    status = platform_chmod("/run/chef/cvd/clients", 0777);
    if (status) {
        fprintf(stderr, "cvd_initialize_server: failed to set mode for /run/chef/cvd/clients\n");
        return status;
    }
#endif
    ...
    if (status) {
        fprintf(stderr, "cvd_initialize_server: error initializing server library %i\n", errno);
        return status;
    }
