...
char* utils_path_local_store_root(void)
{
    ...
    snprintf(
        ...
        "var/chef/local-store"
    );
    return served_paths_path(&buffer[0]);
}

static char* __local_pack_path(const char* publisher, const char* package, int revision)
{
    char buffer[PATH_MAX];
    snprintf(
        &buffer[0], sizeof(buffer),
...
char* utils_path_pack(const char* publisher, const char* package, int revision)
{
    if (revision == 0) {
        return NULL;
    } else if (revision < 0) {
        return __local_pack_path(publisher, package, revision);
    } else {
        char buffer[PATH_MAX];
        snprintf(
            &buffer[0], sizeof(buffer),
            "var/chef/packs/%s-%s.%i.pack",
