...
static size_t __profile_size(uint32_t num_nodes, uint32_t num_edges)
{
...
static protecc_error_t __update_stats_trie_profile(protecc_profile_t* compiled)
{
    size_t num_nodes = 0;
    size_t max_depth = 0;
    size_t num_edges = 0;
    ...
    size_t*                   bytes_written)
{
    ...
    required_size = __profile_size((uint32_t)num_nodes, (uint32_t)num_edges);
    if (required_size > UINT32_MAX) {
        return PROTECC_ERROR_INVALID_ARGUMENT;
    }
...
static protecc_error_t __allocate_import_dfa_buffers(
    ...
    size_t perms_size,
    size_t transitions_size,
    protecc_profile_t** compiled_out)
{
    protecc_profile_t* comp;
