...
static struct {
    ...
    struct ring_buffer* fs_deny_ring;
    struct ring_buffer* net_deny_ring;
    thrd_t deny_thread;
...
static int __deny_event_thread(void* arg)
{
    ...
    while (!g_bpf_manager.deny_thread_stop) {
        int rc;
        if (g_bpf_manager.fs_deny_ring != NULL) {
            rc = ring_buffer__poll(g_bpf_manager.fs_deny_ring, 250);
            if (rc < 0 && rc != -EINTR) {
                VLOG_WARNING("cvd", "bpf_manager: fs deny ring poll failed: %d\n", rc);
                break;
            }
        }
        if (g_bpf_manager.net_deny_ring != NULL) {
            rc = ring_buffer__poll(g_bpf_manager.net_deny_ring, 250);
            if (rc < 0 && rc != -EINTR) {
                VLOG_WARNING("cvd", "bpf_manager: net deny ring poll failed: %d\n", rc);
                break;
            }
        }
    }
...
int containerv_bpf_manager_initialize(void)
{
    ...
    env = getenv("CONTAINERV_BPF_ENABLE_FS");
    if (env && strcmp(env, "0") == 0) {
        enable_fs = 0;
    }
    env = getenv("CONTAINERV_BPF_ENABLE_NET");
    if (env && strcmp(env, "0") == 0) {
        enable_net = 0;
    }

    if (!enable_fs && !enable_net) {
        VLOG_DEBUG("cvd", "bpf_manager: both fs and net LSM programs disabled by env\n");
        return 0;
    }
    ...
    if (enable_fs) {
        g_bpf_manager.fs_skel = fs_lsm_bpf__open();
        if (!g_bpf_manager.fs_skel) {
        ...
        g_bpf_manager.policy_map_fd = bpf_map__fd(g_bpf_manager.fs_skel->maps.policy_map);
        g_bpf_manager.dir_policy_map_fd = bpf_map__fd(g_bpf_manager.fs_skel->maps.dir_policy_map);
        g_bpf_manager.basename_policy_map_fd = bpf_map__fd(g_bpf_manager.fs_skel->maps.basename_policy_map);
        if (g_bpf_manager.policy_map_fd < 0 ||
            g_bpf_manager.dir_policy_map_fd < 0 ||
            g_bpf_manager.basename_policy_map_fd < 0) {
        ...
        if (g_bpf_manager.fs_skel->links.file_open_restrict != NULL) {
            (void)unlink(POLICY_LINK_PIN_PATH);
            status = bpf_link__pin(g_bpf_manager.fs_skel->links.file_open_restrict, POLICY_LINK_PIN_PATH);
        }
        ...
        if (g_bpf_manager.fs_skel->links.bprm_check_security_restrict != NULL) {
            (void)unlink(EXEC_LINK_PIN_PATH);
            status = bpf_link__pin(g_bpf_manager.fs_skel->links.bprm_check_security_restrict, EXEC_LINK_PIN_PATH);
            if (status < 0) {
    ...
    if (enable_net) {
        g_bpf_manager.net_skel = net_lsm_bpf__open();
        if (!g_bpf_manager.net_skel) {
            VLOG_ERROR("cvd", "bpf_manager: failed to open net BPF skeleton\n");
            if (g_bpf_manager.fs_skel != NULL) {
                VLOG_WARNING("cvd", "bpf_manager: continuing with fs LSM only\n");
            } else {
                return -1;
            }
        } else {
            ...
            status = net_lsm_bpf__load(g_bpf_manager.net_skel);
            if (status) {
                VLOG_ERROR("cvd", "bpf_manager: failed to load net BPF skeleton: %d\n", status);
                net_lsm_bpf__destroy(g_bpf_manager.net_skel);
                g_bpf_manager.net_skel = NULL;
                if (g_bpf_manager.fs_skel == NULL) {
                    return -1;
                }
            } else {
                status = net_lsm_bpf__attach(g_bpf_manager.net_skel);
                if (status) {
                    ...
                    g_bpf_manager.net_create_map_fd = bpf_map__fd(g_bpf_manager.net_skel->maps.net_create_map);
                    g_bpf_manager.net_tuple_map_fd = bpf_map__fd(g_bpf_manager.net_skel->maps.net_tuple_map);
                    g_bpf_manager.net_unix_map_fd = bpf_map__fd(g_bpf_manager.net_skel->maps.net_unix_map);
                    if (g_bpf_manager.net_create_map_fd < 0 ||
                        g_bpf_manager.net_tuple_map_fd < 0 ||
                        g_bpf_manager.net_unix_map_fd < 0) {
    ...
    if (g_bpf_manager.fs_skel != NULL) {
        if (g_bpf_manager.fs_skel->maps.deny_events != NULL) {
            int deny_fd = bpf_map__fd(g_bpf_manager.fs_skel->maps.deny_events);
            if (deny_fd >= 0) {
                g_bpf_manager.fs_deny_ring = ring_buffer__new(deny_fd, __deny_event_cb, NULL, NULL);
                if (!g_bpf_manager.fs_deny_ring) {
    ...
    if (g_bpf_manager.net_skel != NULL) {
        if (g_bpf_manager.net_skel->maps.deny_events != NULL) {
            int deny_fd = bpf_map__fd(g_bpf_manager.net_skel->maps.deny_events);
            if (deny_fd >= 0) {
                g_bpf_manager.net_deny_ring = ring_buffer__new(deny_fd, __deny_event_cb, NULL, NULL);
                if (!g_bpf_manager.net_deny_ring) {
    ...
    if (g_bpf_manager.fs_deny_ring != NULL || g_bpf_manager.net_deny_ring != NULL) {
        ...
        if (thrd_create(&g_bpf_manager.deny_thread, __deny_event_thread, NULL) == thrd_success) {
            ...
            if (g_bpf_manager.fs_deny_ring) {
                ring_buffer__free(g_bpf_manager.fs_deny_ring);
                g_bpf_manager.fs_deny_ring = NULL;
            }
            if (g_bpf_manager.net_deny_ring) {
                ring_buffer__free(g_bpf_manager.net_deny_ring);
                g_bpf_manager.net_deny_ring = NULL;
            }
            VLOG_WARNING("cvd", "bpf_manager: failed to start deny ring thread\n");
        }
    }

    g_bpf_manager.available = (g_bpf_manager.fs_skel != NULL || g_bpf_manager.net_skel != NULL);
    ...
    struct containerv_policy* policy)
{
    ...
    int fs_enabled = (g_bpf_manager.policy_map_fd >= 0);
    int net_enabled = (g_bpf_manager.net_create_map_fd >= 0 ||
                       g_bpf_manager.net_tuple_map_fd >= 0 ||
                       g_bpf_manager.net_unix_map_fd >= 0);

    if (!fs_enabled && !net_enabled) {
    ...
    if (fs_enabled && policy->path_count > MAX_PATHS) {
    ...
    if (net_enabled && policy->net_rule_count > 0) {
