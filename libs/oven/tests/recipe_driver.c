...
int main(int argc, char** argv, char** envp)
{
    ...
    struct oven_initialize_options init = {
        ...
        .target_platform = CHEF_PLATFORM_STR,
        .target_architecture = CHEF_ARCHITECTURE_STR,
        .paths = {
            .project_root = argv[2], .source_root = argv[3],
            .build_root = argv[4], .install_root = argv[5],
            .toolchains_root = argv[6], .build_ingredients_root = argv[6]
        }
    };
    ...
    list_foreach(&part->steps, item) {
        ...
        if (step->type == RECIPE_STEP_TYPE_GENERATE) {
            struct oven_generate_options generate = {
                .name = step->name, .system = step->system, .system_options = &step->options,
                .source_dir = step->source_dir, .arguments = &step->arguments,
            ...
            struct oven_build_options build = {
                .name = step->name, .system = step->system, .system_options = &step->options,
                .arguments = &step->arguments, .environment = &step->env_keypairs,
                .skip_generate = step->configure.disabled,
                .source_dir = step->configure.source_dir ? step->configure.source_dir : step->source_dir,
