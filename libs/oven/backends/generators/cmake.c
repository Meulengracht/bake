...
#ifdef _WIN32
#define DEFAULT_INSTALL_PREFIX "Program Files"
#else
...
static char* __add_cmake_prefix(const char* arguments, const char* installPath)
{
...
static char* __replace_or_add_cmake_prefix(const char* arguments, const char* installPath)
{
    ...
    if (!prefix) {
        return __add_cmake_prefix(arguments, installPath);
    }
...
int cmake_main(struct oven_backend_data* data, union oven_backend_options* options)
{
    ...
    newArguments  = __replace_or_add_cmake_prefix(data->arguments, data->install_directory);
