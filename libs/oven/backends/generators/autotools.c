...
// TODO windows not necessary here right?
...
static char* __add_prefix(const char* arguments, const char* installPath, char** prefixPathOut)
{
    // build a new argument string with the prefix added
    char* newArguments = malloc(strlen(arguments) + 1024);
    char* defaultInstallPath = strpathcombine(installPath, DEFAULT_INSTALL_PREFIX);
...
static char* __replace_or_add_prefix(const char* arguments, const char* installPath, char** prefixPathOut)
{
    ...
    if (!prefix) {
        return __add_prefix(arguments, installPath, prefixPathOut);
    }
...
int configure_main(struct oven_backend_data* data, union oven_backend_options* options)
{
    ...
    arguments = __replace_or_add_prefix(data->arguments, data->install_directory, &installPath);
    if (arguments == NULL) {
        return -1;
    }
